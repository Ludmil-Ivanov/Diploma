...
def calibrate_and_warp(image_path):
    ...
    gray_original = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) if len(img.shape) == 3 else img
...
def analyze_part(warped, scale_x=1.0, scale_y=1.0, part_thickness_mm=0.0):
    ...
    shape_type = 'circle' if circ > CIRCULARITY_THRESHOLD else 'rectangle'
    ...
    draw_bx, draw_by = bx, by
    draw_w, draw_h = w_px, h_px
    draw_pcx, draw_pcy = pcx, pcy

    # Z compensation
    CAMERA_HEIGHT_MM = 240.0
    ...
    if shape_type == 'rectangle':
        perfect_cnt = np.array([
            [[int(draw_bx), int(draw_by)]],
            [[int(draw_bx + draw_w), int(draw_by)]],
            [[int(draw_bx + draw_w), int(draw_by + draw_h)]],
            [[int(draw_bx), int(draw_by + draw_h)]]
        ], dtype=np.int32)
        cv2.drawContours(labeled_image, [perfect_cnt], 0, (0, 255, 0), 2)
    else:
        radius = (draw_w + draw_h) / 4.0
        cv2.circle(labeled_image, (int(draw_pcx), int(draw_pcy)), int(radius), (0, 255, 0), 2)

    # G54 origin marking (X0 Y0)
    origin_x, origin_y = int(pcx), int(pcy)
    cv2.drawMarker(labeled_image, (origin_x, origin_y), (0, 0, 255), markerType=cv2.MARKER_CROSS, markerSize=90,
                   thickness=6)
